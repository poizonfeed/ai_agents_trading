...
Run `analyst_agent` per its `agents/analyst_agent/CLAUDE.md`, passing `{run_folder}`.
...
Run `strategy_agent` per its `agents/strategy_agent/CLAUDE.md`, passing `{run_folder}`.
